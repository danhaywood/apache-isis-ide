 ...
 * 
 */
public class NakedObjectPropertiesContentProvider 
		extends AbstractNodesContentProvider {

	private final static Logger LOGGER = Logger.getLogger(NakedObjectPropertiesContentProvider.class);
	@Override
	public Logger getLOGGER() {
		return LOGGER;
	}

	public NakedObjectPropertiesContentProvider(ICompilationUnitOwner compilationUnitOwner) {
	...
	protected Object[] doGetElements(TypeDeclaration typeDeclaration) {
		...
		for(Object bodyDeclarationObj: typeDeclaration.bodyDeclarations()) {
			...
			String propertyName = MethodNameUtils.asPropertyName(methodDeclaration);
			if (propertyName == null) {
				continue;
			}
			
			NakedObjectProperty nakedObjectProperty = membersByName.get(propertyName);
			if (nakedObjectProperty == null) {
				nakedObjectProperty = new NakedObjectProperty(
					getCompilationUnitOwner().getCompilationUnit(), 
					propertyName, methodDeclaration);
				membersByName.put(nakedObjectProperty.getAccessorMethodName(), nakedObjectProperty);
			}
		}
